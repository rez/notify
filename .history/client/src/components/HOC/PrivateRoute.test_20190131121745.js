...
describe('Private Route HOC', () => {
    ...
    it('should not render while fetching', () => {
        ...
        const PrivateRoute = withAuth(Dashboard);
    ...
    it('should load protected page if logged in', () => {
        const store = mockStore({auth : {fetching : false, auth : fixtures.user, dashboard : defaultState}});
        const PrivateRoute = withAuth(Dashboard);
        const wrapper = mount(<Provider store={store}><PrivateRoute  /></Provider>);
        console.log(wrapper.find('Connect(PrivateRoute)').dive().debug())
       // expect(wrapper.find('Redirect').exists()).toBe(true);
    });
});