...
describe('Private Route HOC', () => {
    ...
    it('should not render while fetching', () => {
        ...
        console.log(wrapper.find("Connect(PrivateRoute)").shallow().debug()); 
    ...
    it('should load protected page if logged in', () => {
        const store = mockStore({dashboard:fixtures.dashboard, auth : {fetching : false}});
        const PrivateRoute = withAuth(DashboardContainer);
        const wrapper = shallow(<Provider store={store}><BrowserRouter><PrivateRoute  /></BrowserRouter></Provider>).shallow();
        console.log(wrapper.state())
        expect(wrapper.state().renderRoute).toBe(true);
