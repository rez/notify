...
describe('Player', () => {
    ...
    describe('when track is available', () =>{
        ...
        it('artist name rendered', () => {
            expect(player.find('.MetaArtist').html()).toEqual(`<span class="MetaArtist">${fixtures.track.name}</span>`);
        });
        it('title rendered', () => {
            expect(player.find('.MetaTitle').html()).toEqual(`<span class="MetaTitle">${fixtures.track.album.artists[0].name}</span>`);
        });
        it('no meta IMG rendered', () => {
