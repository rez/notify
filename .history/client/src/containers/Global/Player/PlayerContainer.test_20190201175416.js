...
describe('Private Route HOC', () => {
    ...
    it('should start up player state service', () => {
        const spotifyPollMock = jest.fn(() => {
            props.requestPlayerState();
        });
       ...
       //const spy2 = spyOn(wrapper.instance(),'requestPlayerState');
        ...
        wrapper.setProps({ auth: { auth: fixtures.user }, requestPlayerState : jest.fn()});
        wrapper.setState({position:20});
    ...
    it('should render dashboard', () => {
        const wrapper = shallow(<PlayerContainer {...props} auth={{auth : fixtures.user}} player={defaultState} />, { disableLifecycleMethods: false });
