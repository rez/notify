...
describe('Private Route HOC', () => {
    ...
    it('load initial content after location decided', () =>{
        wrapper.setProps({location : {...defaultLocationState, init : true}});
        const spy = spyOn(wrapper.instance(),'updateActiveFilter');
        wrapper.instance().componentDidUpdate();
        expect(spy).toHaveBeenCalled();


    });
    it('loads content after content has previously loaded', () => {
        wrapper.setState(state => {
            const dashNavFilter = wrapper.state('dashNavFilter').map(filter => {
                filter.loaded = true;
                return filter;
            });
        });
        wrapper.setProps({match : {params : {filter : constants.FOLLOWING_LINK}}});
        wrapper.instance().componentDidUpdate();
        expect(props.updateActiveFilter.calledOnce).toBe(true);
    });
