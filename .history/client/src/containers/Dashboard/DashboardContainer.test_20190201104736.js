...
describe('Private Route HOC', () => {
    ...
    it('handle artist release clicked', () => {
       ...
       const newState = {activeReleases : fixtures.releases[0].releases, 
        activeReleasesArtist :"Testy ", 
       showSetsModal : false, 
        showReleasesModal : true};
       // expect(object)
        console.log(Object.keys(newState).filter(x => wrapper.state()[x] === newState[x]).length == Object.keys(newState).length);
       //console.log(Object.keys(wrapper.state()).reduce((acc,obj) =>{acc[obj] = wrapper.state()[obj]; return acc}, []));
       //Object.keys(wrapper.state()).filter(x > wrapper.state()[x] === )
      // [...wrapper.state()].filter(x => )
    //    expect(Object.keys(wrapper.state()).reduce((acc,obj) =>{acc[obj] = wrapper.state()[obj]; return acc}, []).includes([activeReleases : fixtures.releases[0].releases, 
